...
async def cmd_list(event):
    ...
    if input_str == "text":
        string = (
            "Total {count} commands found in {plugincount} plugins of HyperUserBot-X\n\n"
        )
        catcount = 0
        plugincount = 0
        ...
        if len(string) > 4095:
            ...
            reply_text = f"**All commands of the HyperUserBot-X can be seen [here]({url})**"
...
async def info(event):
    ...
    input_str = event.pattern_match.group(1)
    if input_str == "text":
        string = "Total {count} commands found in {plugincount} sudo plugins of HyperUserBot-X\n\n"
        catcount = 0
        plugincount = 0
        ...
        if len(string) > 4095:
            ...
            url = f"https://nekobin.com/{key}"
            reply_text = f"All commands of the HyperUserBot-X are [here]({url})"
